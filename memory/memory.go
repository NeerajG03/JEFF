...
//
// curateCmd is the agent binary name (e.g. "claude", "gemini").
// curateArgs builds the non-interactive arguments (e.g. ["--dangerously-skip-permissions", "-p", prompt]).
// If curateArgs is nil, auto-curation is not supported for this agent.
func AutoCurate(taskDir, taskID, personaName, jeffHome string, repos []string, curateCmd string, curateArgs []string) error {
	...
	if curateArgs == nil {
		return nil // agent doesn't support non-interactive mode
	}

	// Launch agent in non-interactive mode with the curation prompt.
	cmd := exec.Command(curateCmd, curateArgs...)
	cmd.Dir = taskDir
...
// InstallLearnCommandTo writes the /learn command for a given config layout.
// configDir is the agent config dir (e.g. ".claude", ".gemini").
// commandsSubdir is the commands subdirectory (e.g. "commands").
// fileExt is the file extension (e.g. "md", "toml").
func InstallLearnCommandTo(taskDir, taskID, personaName, jeffHome string, repos []string, configDir, commandsSubdir, fileExt string) error {
	if commandsSubdir == "" {
		return nil // agent doesn't support custom commands
	}

	dir := filepath.Join(taskDir, configDir, commandsSubdir)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return fmt.Errorf("create commands dir: %w", err)
	}

	content := buildLearnContent(taskDir, taskID, personaName, jeffHome, repos)

	filename := "learn." + fileExt
	path := filepath.Join(dir, filename)

	if fileExt == "toml" {
		// TOML format: wrap in prompt field.
		tomlContent := "# Auto-generated by jeff — /learn command\n\nprompt = \"\"\"\n" + content + "\"\"\"\n"
		return os.WriteFile(path, []byte(tomlContent), 0o644)
	}
	if configDir == ".opencode" {
		content = "---\ndescription: Curate task observations into JEFF memory\n---\n\n" + content
	}
...
func InstallLearnCommand(taskDir, taskID, personaName, jeffHome string, repos []string) error {
	return InstallLearnCommandTo(taskDir, taskID, personaName, jeffHome, repos, ".claude", "commands", "md")
}

func buildLearnContent(taskDir, taskID, personaName, jeffHome string, repos []string) string {
